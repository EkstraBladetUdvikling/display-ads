  ...
  window.addEventListener('message', (event) => {
    if (typeof event.data === 'string' && event.data.indexOf('high-impact-js') !== -1) {
      if (event.data.indexOf('AD_RENDERED') !== -1 && event.data.indexOf('origins') !== -1) {
        ...
        window.ebTracking.queryBuilder('event', {
          el1: 'advertising',
          el2: 'banner_load',
          el3: 'highimpact',
          el4: data.origins.length ? data.origins[data.origins.length - 1] : 'unknown',
        });
      }
    }
  });
...
export function addHighImpact(banner: IBANNERSTATEBANNER, adUnitId: string) {
  ...
  if (banner.cleanName?.indexOf('topscroll') !== -1) {
    ...
    if (banner.cleanName?.indexOf('monster') !== -1 && banner.cleanName?.indexOf('artikel') === -1) {
      window.highImpactJs.cmd.push(() => {
    ...
    if (banner.cleanName?.indexOf('swedishtakeover') !== -1 || banner.cleanName?.indexOf('swedish_artikel') !== -1) {
      window.highImpactJs.cmd.push(() => {
        const sizes = BANNERSTATE.device === 'desktop' ? [[970, 570]] : [[300, 240]];
        if (window.highImpactJs.defineSlot) {
          window.highImpactJs.defineSlot({
